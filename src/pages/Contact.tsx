          ...
          <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="space-y-4">
              <div className="card p-6">
                <h3 className="font-display font-bold text-navy-900 dark:text-white mb-4">Contact Information</h3>
                <div className="space-y-3">
                  <a href={`mailto:${COMPANY.email}`} className="flex items-center gap-3 text-sm text-navy-600 dark:text-navy-300 hover:text-navy-600 dark:hover:text-navy-400 transition-colors">
                    <div className="w-9 h-9 rounded-lg bg-navy-50 dark:bg-navy-500/10 flex items-center justify-center"><Mail className="w-4 h-4 text-navy-600 dark:text-navy-400" /></div>
                    {COMPANY.email}
                  </a>
                  <a href={WHATSAPP_LINK} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-sm text-navy-600 dark:text-navy-300 hover:text-navy-600 dark:hover:text-navy-400 transition-colors">
                    <div className="w-9 h-9 rounded-lg bg-navy-50 dark:bg-navy-500/10 flex items-center justify-center"><Phone className="w-4 h-4 text-navy-600 dark:text-navy-400" /></div>
                    {COMPANY.phone}
                  </a>
                  <div className="flex items-center gap-3 text-sm text-navy-600 dark:text-navy-300">
                    <div className="w-9 h-9 rounded-lg bg-navy-50 dark:bg-navy-500/10 flex items-center justify-center"><MapPin className="w-4 h-4 text-navy-600 dark:text-navy-400" /></div>
                    {COMPANY.location}
                  </div>
                  <div className="flex items-center gap-3 text-sm text-navy-600 dark:text-navy-300">
                    <div className="w-9 h-9 rounded-lg bg-navy-50 dark:bg-navy-500/10 flex items-center justify-center"><Globe className="w-4 h-4 text-navy-600 dark:text-navy-400" /></div>
              ...
              <div className="card p-6">
                <div className="flex items-center gap-2 mb-4">
                  <Clock className="w-5 h-5 text-navy-600 dark:text-navy-400" />
                  <h3 className="font-display font-bold text-navy-900 dark:text-white">Business Hours</h3>
                </div>
                <div className="space-y-2">
                  {BUSINESS_HOURS.map((bh) => (
                    <div key={bh.day} className="flex items-center justify-between text-sm">
                      <span className="text-navy-600 dark:text-navy-300">{bh.day}</span>
                      <span className={`font-medium ${bh.hours === 'Closed' ? 'text-red-500' : 'text-navy-900 dark:text-white'}`}>{bh.hours}</span>
                    </div>
                  ))}
                </div>
                <div className="mt-4 pt-4 border-t border-navy-100 dark:border-navy-800 flex items-center gap-2 text-sm">
                  <Zap className="w-4 h-4 text-navy-600 dark:text-navy-400" />
                  <span className="text-navy-600 dark:text-navy-300">Response time: <span className="font-semibold text-navy-900 dark:text-white">{RESPONSE_TIME}</span></span>
                </div>
              </div>

              {/* WhatsApp CTA */}
              <div className="card p-6 bg-gradient-to-br from-navy-900 to-navy-800 text-white border-0">
                <h3 className="font-display font-bold mb-2">Prefer to talk now?</h3>
                <p className="text-sm text-navy-200 mb-4">Message us on WhatsApp and we'll respond right away.</p>
            ...
            <div className="lg:col-span-2 space-y-6">
              <div className="card p-6 lg:p-8">
                {/* Mode toggle */}
                <div className="flex gap-2 mb-6 p-1 rounded-xl bg-navy-50 dark:bg-navy-800">
                  <button
                    onClick={() => setMode('contact')}
                    className={`flex-1 px-4 py-2.5 rounded-lg text-sm font-semibold transition-all ${mode === 'contact' ? 'bg-white dark:bg-navy-700 text-navy-900 dark:text-white shadow-sm' : 'text-navy-500 dark:text-navy-400'}`}
                  >
                  ...
                  <button
                    ...
                    className={`flex-1 px-4 py-2.5 rounded-lg text-sm font-semibold transition-all ${mode === 'consultation' ? 'bg-white dark:bg-navy-700 text-navy-900 dark:text-white shadow-sm' : 'text-navy-500 dark:text-navy-400'}`}
                  >
                ...
                {status === 'success' ? (
                  <div className="text-center py-12">
                    ...
                    <h3 className="font-display font-bold text-navy-900 dark:text-white text-xl mb-2">
                  ...
                  <form onSubmit={submit} className="space-y-4">
                    <div className="grid sm:grid-cols-2 gap-4">
                      ...
                      <div>
                        <label className="block text-sm font-medium text-navy-700 dark:text-navy-200 mb-1.5">Email *</label>
                        <input required type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className="input-field" />
                      </div>
                    </div>
                    <div className="grid sm:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-medium text-navy-700 dark:text-navy-200 mb-1.5">Company</label>
                        <input value={form.company} onChange={(e) => setForm({ ...form, company: e.target.value })} className="input-field" />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-navy-700 dark:text-navy-200 mb-1.5">Phone</label>
                        <input value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} className="input-field" />
                      </div>
                    </div>
                    <div className="grid sm:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-medium text-navy-700 dark:text-navy-200 mb-1.5">Service of Interest</label>
                      ...
                      {mode === 'consultation' && (
                        <div>
                          <label className="block text-sm font-medium text-navy-700 dark:text-navy-200 mb-1.5">Preferred Date</label>
              ...
              <div className="card overflow-hidden">
                <div className="relative h-64 bg-navy-100 dark:bg-navy-800 flex items-center justify-center">
                  <iframe
                ...
                <div className="p-4 flex items-center gap-3">
                  <MapPin className="w-5 h-5 text-navy-600 dark:text-navy-400 flex-shrink-0" />
                  <div>
                    <div className="font-semibold text-navy-900 dark:text-white text-sm">{COMPANY.location}</div>
                    <div className="text-xs text-navy-500 dark:text-navy-400">{COMPANY.serviceCoverage} · {COMPANY.serviceCoverageSub}</div>
                  </div>
                </div>
              </div>
            </div>
