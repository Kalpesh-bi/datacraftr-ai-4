          ...
          <div className="grid lg:grid-cols-3 gap-6 max-w-5xl mx-auto">
            {pricingPlans.map((plan, i) => (
              <motion.div
                ...
                className={`card p-8 relative ${plan.popular ? 'border-navy-500 ring-2 ring-navy-500/20 lg:scale-105' : ''}`}
              >
                {plan.popular && (
                ...
                <h3 className="font-display font-bold text-navy-900 text-lg mb-1">{plan.name}</h3>
                <p className="text-sm text-navy-500 mb-4">{plan.description}</p>
                <div className="mb-6">
                  <span className="font-display text-4xl font-bold text-navy-900">{plan.price}</span>
                  {plan.period && <span className="text-navy-500 ml-1">{plan.period}</span>}
                </div>
